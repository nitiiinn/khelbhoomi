...
def open_android_camera():
    ...
    backends = [cv2.CAP_ANDROID, cv2.CAP_ANY]
            ...
            try:
                ...
                if cap is not None and cap.isOpened():
                    ...
                    if ret:
                        ...
                        print(f"[INFO] Android camera opened successfully on index {idx}")
                        return cap
                
                cap.release()
            except Exception as e:
                print(f"[ERROR] Failed to open camera {idx}: {e}")
                continue

    print("[ERROR] Failed to open Android camera with all attempts")
    return None
        ...
        if platform == "android":
            ...
            def permission_callback(permissions, grant_results):
                if all(grant_results):
                    ...
                    print("[WARNING] Some permissions denied")
